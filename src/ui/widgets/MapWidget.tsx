...
export default function MapWidget() {
  ...
  useEffect(() => {
    ...
    let handle;
    try {
      handle = createMap(ref.current, { center: [37.8, -96], zoom: 4 });
      if (!handle) {
        console.error(
          'MapWidget: map initialization failed - createMap returned null',
        );
        return;
      }
    } catch (error) {
      console.error('MapWidget: map initialization failed', error);
      return;
    }

    // геолокация → центр (без ошибок и гонок)
    if (navigator.geolocation && handle) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          try {
            ...
            console.error('MapWidget: geolocation setup failed', error);
          }
        },
        () => {
